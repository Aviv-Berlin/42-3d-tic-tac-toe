...
export async function getUserWins(id: number) {
	const result = await query(
		'SELECT COUNT(*) FROM matches WHERE (player1 = $1 OR player2 = $1) AND winner = $1 AND player2 != 1;', [id]
	);
...
export async function getUserDraws(id: number) {
	const result = await query(
		'SELECT COUNT(*) FROM matches WHERE (player1 = $1 OR player2 = $1) AND winner IS null AND player2 != 1;', [id]
	);
	return Number(result.rows[0].count);
}
...
export async function getUserLosses(id: number) {
	const result = await query(
		'SELECT COUNT(*) FROM matches WHERE (player1 = $1 OR player2 = $1) AND winner IS NOT null AND winner != $1 AND player2 != 1;', [id]
	);
	return Number(result.rows[0].count);
}
